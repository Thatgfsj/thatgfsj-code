...
- **零工具轮的 success:true 无法与纯问答区分**：模型一步不做就宣布完成
  （"不作为型假成功"）在协议层与合法问答同形；`--json` 消费方应以
  `stats.toolCalls`（0 次即存疑）自行加权判断。
- **小高度（约 24 行）下超限输出按"保尾裁头"裁剪**，并保留原文首行作
  标题——这是预算设计的一致推论（尾部锚定优先保最新内容），非缺陷。
